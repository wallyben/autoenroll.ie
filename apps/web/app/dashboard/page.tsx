...
export default function DashboardPage() {
  ...
  const analytics = useSWR(token ? [`${apiBase}/analytics`, token] : null, ([url, t]) => fetcher(url, t));
  ...
  return (
    <main className="space-y-6">
      ...
      <div className="card space-y-3">
        ...
        {analytics.data ? (
          <pre className="text-xs whitespace-pre-wrap">{JSON.stringify(analytics.data, null, 2)}</pre>
        ) : (
